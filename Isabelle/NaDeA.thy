...
proof (induct t and l rule: semantics_term.induct semantics_list.induct) qed auto                   (*  occurring inside the term.                                                *)
                                                                                                    (*                                                                            *)
...
lemma alltail: "all b (p # z) ==> all b z" proof simp qed                                           (*  "all" holds for tail of list.                                             *)
                                                                                                    (*                                                                            *)
lemma allnew: "all (new c) z = news c z" proof (induct z) qed (auto, metis)                         (*  "all new" corresponds to "news".                                          *)
                                                                                                    (*                                                                            *)
...
proof (induct p arbitrary: e) qed (auto simp add: map'(2), metis+)                                  (*       when the value c is new in the formula.                              *)
                                                                                                    (*                                                                            *)
...
proof (induct t and l rule: semantics_term.induct semantics_list.induct) qed auto                   (*               in the environment.                                          *)
                                                                                                    (*                                                                            *)
...
proof (induct p arbitrary: e v t)                                                                   (*                                                                            *)
  ...
  have "semantics e f g (sub v t (Exi p)) =                                                         (*  Holds for formulas within scope of existential quantification.            *)
      (? x. semantics (put (put e 0 x) (v + 1) (semantics_term (put e 0 x) f (inc_term t))) f g p)" (*  Substitution within "Exi" can be obtained through "put" and "inc_term".   *)
    using * proof simp qed                                                                          (*                                                                            *)
  also have "... = (? x. semantics (put (put e v (semantics_term e f t)) 0 x) f g p)"               (*  Using lemmas "commute" and "increment", we can reduce it to only "put".   *)
    using commute increment(1) proof metis qed                                                      (*                                                                            *)
  finally show "semantics e f g (sub v t (Exi p)) =                                                 (*  Adding Exi p we only have to use one put, and the lemma is                *)
      semantics (put e v (semantics_term e f t)) f g (Exi p)" proof simp qed                        (*  shown to hold for formulas with Exi p structure for arbitrary p.          *)
  have "semantics e f g (sub v t (Uni p)) =                                                         (*  Holds for universal quantification. Same as for Exi p.                    *)
...
qed (auto simp add: substitute'(2))                                                                 (*  The remaining cases are proved automatically.                             *)
                                                                                                    (*                                                                            *)
...
next                                                                                                (*                                                                            *)
  fix f p q z r assume "all (semantics e f g) z"                                                    (*  Case: Disjunction Introduction.                                           *)
...
next                                                                                                (*  Case: Conjunction Introduction.                                           *)
...
qed (auto simp add: substitute)                                                                     (*  Remaining cases are proved automatically by auto/simp and substitute.     *)
                                                                                                    (*                                                                            *)
